        ...
        public class ICEUDPTransport: JingleTransport, @unchecked Sendable {
            ...
            public class Candidate: CustomElementConvertible, @unchecked Sendable {
                ...
                public convenience init?(from el: Element) {
                    ...
                    let relAddr = el.attribute("rel-addr");
                    let relPort = UInt16(el.attribute("rel-port") ?? "")
                    
                    let type = CandidateType(rawValue: el.attribute("type") ?? "");
                    self.init(component: component, foundation: foundation, generation: generation, id: id, ip: ip, network: 0, port: port, priority: priority, protocolType: proto, relAddr: relAddr, relPort: relPort, type: type, tcpType: el.attribute("tcptype"));
                }
