...
extension MixModule {
    ...
    open func retrieveConfig(for channelJid: BareJID) async throws -> JabberDataElement {
        return try await withUnsafeThrowingContinuation { continuation in
    ...
    open func updateConfig(for channelJid: BareJID, config: JabberDataElement) async throws {
        return try await withUnsafeThrowingContinuation { continuation in
    ...
    open func retrieveHistory(fromChannel jid: BareJID, max: Int) async throws {
        var complete = false;
        var rsm = RSM.Query(lastItems: max);
        while !complete {
            let result = try await self.retrieveHistory(fromChannel: jid, start: nil, rsm: rsm);
            if let nextRsm = result.rsm?.next() {
                rsm = nextRsm;
    ...
    open func retrieveHistory(fromChannel jid: BareJID, start: Date, after: String?) async throws {
        var complete = false;
        var rsm = RSM.Query(before: nil, after: after, max: 100);
        while !complete {
            let result = try await self.retrieveHistory(fromChannel: jid, start: start, rsm: rsm);
            if let nextRsm = result.rsm?.next() {
                rsm = nextRsm;
                complete = result.complete;
            } else {
